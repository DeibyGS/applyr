...
def find_cv_for_offer(offer_id: int, cv_used: str | None) -> Path | None:
    """The CV file `cv generate` wrote for an offer, or None if it is gone.
    ...
    `cv keywords` told users to run the generate step they had just run.
    `cv_used` is the authoritative link, so read it first and keep the glob
    as a fallback for hand-named files. Shared with `applyr next` (ADR-015).
    """
    cv_dir = get_output_dir()
    if cv_used:
        stem = Path(cv_used).stem
        found = next((p for ext in (".md", ".html") if (p := cv_dir / f"{stem}{ext}").exists()), None)
        if found is not None:
            return found
    cv_files = sorted(cv_dir.glob(f"*offer_{offer_id}*"))
    return cv_files[0] if cv_files else None
...
def _record_and_report(offer_id: int, step: str, score: int, as_json: bool) -> None:
    """Persist a `--record` score (ADR-015) and confirm it — shared by both review commands."""
    import json

    from applyr.pipeline_next import record_review

    entry = record_review(offer_id, step, score, load_config())
    if as_json:
        print(json.dumps({"offer_id": offer_id, **entry}, ensure_ascii=False))
    else:
        print(f"Recorded {step} for offer #{offer_id}: {entry['score']} → {entry['verdict']}")
        print(f"  Next: applyr next {offer_id}")


def cmd_cv_review(cv_file: str, as_json: bool = False, record: str | None = None) -> None:
    """Generate a recruiter-review prompt for a CV file (markdown or HTML).

    With `record`, instead store the score of a review the agent already
    executed against the offer embedded in the file (ADR-015).
    """
    import json
    ...
    if score is not None:
        ...
        if offer_id is None:
            die("Error: no offer id found in this CV — pass a file generated by 'applyr cv generate'.",
                code="no_offer_id")
        _record_and_report(offer_id, "cv_review", score, as_json)
        return
...
def cmd_cv_review_blind(offer_id: int, as_json: bool = False, record: str | None = None) -> None:
    ...
    against the offer, and returns an ATS assessment with verdict. With
    `record`, instead store the score of the prompt the agent executed (ADR-015).
    """
    import json
    ...
    if record is not None:
        _record_and_report(offer_id, "review_blind", parse_record_score(record), as_json)
        return
...
def cmd_cv_keywords(offer_id: int, as_json: bool = False) -> None:
    ...
    cv_path = find_cv_for_offer(offer_id, offer_data.get("cv_used"))
    if cv_path is None:
